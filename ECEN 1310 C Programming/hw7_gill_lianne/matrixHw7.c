...
void testSymmetric(void) {
  ...
  err = setElement(B, 1, 2, .25); assert(!err);
  err = setElement(B, 1, 3, -.1); assert(!err);
  err = setElement(B, 2, 2, .4); assert(!err);
  err = setElement(B, 2, 3, .3); assert(!err);
  err = setElement(A, 3, 2, .1); assert(!err);
  err = setElement(A, 3, 3, -.3); assert(!err);
  result = symmetric(B);
  printf("Matrix B:\n");
  ...
  err = setElement(C, 1, 1, 1); assert(!err);
  err = setElement(C, 1, 2, 0); assert(!err);
  err = setElement(C, 1, 3, 0); assert(!err);
  err = setElement(C, 2, 1, 0); assert(!err);
  err = setElement(C, 2, 2, 1); assert(!err);
  err = setElement(C, 2, 3, 0); assert(!err);
  err = setElement(C, 3, 1, 0); assert(!err);
  err = setElement(C, 3, 2, 0); assert(!err);
  err = setElement(C, 3, 3, 1); assert(!err);
  result = symmetric (C);
  printf("Matrix C:\n");
  err = printMatrix(C);
  assert(result);
...
void testGetColumnGetRow(void) {
 ...
 assert(B);
  ...
  err = setElement(B, 2, 2, .4); assert(!err);
  err = setElement(B, 2, 3, .3); assert(!err);
  err = setElement(B, 3, 2, .1); assert(!err);
  err = setElement(B, 3, 3, -.3); assert(!err);
  printf("Matrix B:\n");
  err = printMatrix(B); assert(!err);
  ...
  c = getColumn (B, 2);
  for ( i=1; i <= 3; i++) {
    assert (getE (B, i, 2) == getE(c,i,1));
  }
  r =getRow (B, 1);
  for ( i=1; i <= 3; i++) {
    assert (getE (B, i, 1)== getE(r,1,i));
  }
  deleteMatrix (B);
}
...
void testScalarProduct(void) {
 ...
 assert(B);
  ...
  err = setElement(B, 3, 2, .1); assert(!err);
  err = setElement(B, 3, 3, -.3); assert(!err);
  matrix * A = copyMatrix(B);
  A = scalarProduct(3 , A);
  for (i = 1; i < 3; i++) {
    for (j =1; j< 3; j++) {
      assert (3 * getE(B, i, j) == getE(A, i, j));
    }
  }  
}
...
int diagonal(matrix const * v, matrix * mtx) {
 ...
 int symmetric(matrix const * mtx){
    ...
    int row, col;
    for (row =1; row <= mtx -> rows; row ++) {
      for (col = row+1; col<= mtx->cols; col ++) {
...
matrix * getColumn(matrix const * mtx, int col) {
    if (mtx == NULL || col <=0) return 0;
    matrix * m = newMatrix (mtx -> rows, 1);
      int row;
    for (row =1; row <= mtx -> rows; row++){
      setE(m, row, 1, getE(mtx, row, col));
    }
    return m;
}
  matrix * getRow(matrix const * mtx, int row) {
    if (mtx == NULL || row <=0) return 0;
    matrix * n = newMatrix (1, mtx -> cols);
    int col;
    for (col = 1; row<= mtx -> cols; col++){
      setE(n, col, 1, getE(mtx, row, col));
    }
    return n;
 }
int scalarProduct(double s, matrix * mtx) {
  ...
  for (i = 1; i < mtx -> rows; i++){
    for (j=1; i< mtx -> cols; j++){
      setE(mtx, i,j, s* getE(mtx, i, j));
    }
  }
  return 0;
}
